  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="space-y-6 p-6">
        <h1 className="text-2xl font-bold">Agenda personal</h1>
...
// import AppLayout from '@/layouts/app-layout';
// import type { BreadcrumbItem, Task } from '@/types/agenda';
// import { Head } from '@inertiajs/react';
...
// export default function AgendaPage({ initialTasks }: AgendaPageProps) {
//   // const { tasks, deleteTask } = useAgenda({ initialTasks });

//   // const openDialog = (date?: Date, task?: Task) => {
//   //   // Este openDialog se maneja dentro de AgendaCalendar, pero si quieres abrir desde TaskList, puedes pasar una función callback
//   //   // Por simplicidad, lo manejamos todo en AgendaCalendar
//   // };
...
// export default function Agenda({ initialTasks }: AgendaProps) {
//   const [currentDate, setCurrentDate] = useState(new Date());
//   const [viewMode, setViewMode] = useState<ViewMode>('month');
//   const [tasks, setTasks] = useState<Task[]>(initialTasks);
//   const [isDialogOpen, setIsDialogOpen] = useState(false);
...
//     try {
//       if (editingTask) {
//         const { data } = await axios.put(`/agenda/${editingTask.id}`, payload);
//         setTasks((prev) =>
//           prev.map((t) => (t.id === editingTask.id ? data : t)),
//         );
//       } else {
//         const { data } = await axios.post('/agenda', payload);
...
//     try {
//       const { data } = await axios.put(`/agenda/${draggedTask.id}`, {
//         date: dateStr,
//       });
//       setTasks((prev) => prev.map((t) => (t.id === draggedTask.id ? data : t)));
//     } catch (error) {
