...
export default function Index() {
  ...
  const [cuotas, setCuotas] = useState<
    Omit<PagarCuota, 'id' | 'cuenta_pagar_id'>[]
  >([]);
  ...
  const [currentCuotasList, setCurrentCuotasList] = useState<PagarCuota[]>([]);
  ...
  const handleEdit = (cuenta: CuentaPagar) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      router.put(`/cuentasporpagar/${editingId}`, formData, {
        onSuccess: () => {
          setOpen(false);
          reset();
          setEditingId(null);
      ...
      router.post('/cuentasporpagar', formData, {
        onSuccess: () => {
          setOpen(false);
          reset();
          setCuotas([]);
  ...
  const handleSaveCuotas = () => {
    if (selectedCuentaId) {
      ...
      //   },
      // });
      router.post(`/cuentasporpagar/${selectedCuentaId}/cuotas`, formData, {
        onSuccess: () => {
          setOpenCuotas(false);
          setCuotas([]);
          setSelectedCuentaId(null);
  ...
  const handleOpenCobrar = (cuenta: CuentaPagar) => {
  ...
  const handleSubmitPago = (e: React.FormEvent) => {
    ...
    if (selectedCuentaId) {
      ...
      //   },
      // });
      router.post(`/cuentasporpagar/${selectedCuentaId}/pagar`, formData, {
        onSuccess: () => {
          setOpenCobrar(false);
          setSelectedCuentaId(null);
          setSelectedCuotasToPay([]);
          ...
          <Dialog open={open} onOpenChange={handleOpenChange}>
            ...
            >
              <DialogHeader>
                ...
                <DialogDescription>
                  {editingId
                    ? 'Modifica los datos de la cuenta por pagar'
                    : 'Completa los datos para crear una nueva cuenta por pagar'}
                </DialogDescription>
              </DialogHeader>
          ...
          >
            ...
            <form onSubmit={handleSubmitPago}>
              ...
              <DialogFooter className="mt-4">
                ...
                >
                  {processingPago ? 'Procesando...' : 'Registrar Pago'}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
...
//             Gestión de Cuentas por Pagar
//           </div>
//           <Link href="/cuentasporpagar/create">
//             <Button>
//               <Plus className="mr-2 h-4 w-4" /> Nuevo
//             </Button>
//           </Link>
...
//                   <TableCell>
//                     <Link href={`/cuentasporpagar/edit/${compra.id}`}>
